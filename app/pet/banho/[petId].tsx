...
export default function BanhoScreen() {
  const router = useRouter();
  const { petId } = useLocalSearchParams();
  const [pet, setPet] = useState<Pet | null>(null);
  const [petSelectorVisible, setPetSelectorVisible] = useState(false);
  const [expandedBanhoId, setExpandedBanhoId] = useState<string | null>(null);
  ...
  const banhos: Banho[] = [
    {
      ...
      avatar: require('../../../assets/images/BanhoImg.png'),
      data: '15.04.25',
      status: 'Em andamento',
      statusColor: '#4A90E2'
    },
  ...
  const banhosFiltrados = banhos.filter(b => b.petId === idParam);
  ...
  const renderBanhoItem = ({ item }: { item: Banho }) => {
    const isExpanded = expandedBanhoId === item.id;
    return (
      <View style={styles.banhoItem}>
        <TouchableOpacity
          onPress={() => setExpandedBanhoId(isExpanded ? null : item.id)}
          activeOpacity={0.8}
          style={{ flexDirection: 'row', alignItems: 'center' }}
        >
          <Image source={item.avatar} style={styles.profissionalAvatar} />
          <View style={{ flex: 1, marginLeft: 12 }}>
            <Text style={styles.banhoTipo}>{item.tipo}</Text>
            <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 4 }}>
              <Ionicons name="calendar-outline" size={14} color="#717786" />
              <Text style={styles.banhoData}>{item.data}</Text>
            </View>
          </View>
        ...
        {isExpanded && (
          <View style={{ alignItems: 'center', marginTop: 12, width: '100%' }}>
            <Image 
              source={require('../../../assets/images/BanhoDetalhes.png')} 
              style={styles.banhoDetalheImage} 
            />
          </View>
        )}
      </View>
    ...
    <>
      ...
      <SafeAreaView style={styles.container}>
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          <Text style={styles.sectionTitle}>Histórico de banhos</Text>
          {banhosFiltrados.length === 0 ? (
            <Text style={{ textAlign: 'center', color: '#717786', marginTop: 24 }}>
              Nenhum banho encontrado para este pet.
            </Text>
          ) : (
            banhosFiltrados.map((banho) => (
              <View key={`banho-${banho.id}`} style={styles.banhoItem}>
                <TouchableOpacity
                  onPress={() => setExpandedBanhoId(expandedBanhoId === banho.id ? null : banho.id)}
                  activeOpacity={0.8}
                  style={{ flexDirection: 'row', alignItems: 'center' }}
                >
                  <Image source={banho.avatar} style={styles.profissionalAvatar} />
                  <View style={{ flex: 1, marginLeft: 12 }}>
                    <Text style={styles.banhoTipo}>{banho.tipo}</Text>
                    <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 4 }}>
                      <Ionicons name="calendar-outline" size={14} color="#717786" />
                      <Text style={styles.banhoData}>{banho.data}</Text>
                    </View>
                  </View>
                  <View style={{ alignItems: 'flex-end', minWidth: 90 }}>
                    <Text style={[styles.statusText]}>{banho.status}</Text>
                    {banho.status === 'Em andamento' ? (
                      <View style={{ width: 60, height: 4, borderRadius: 2, backgroundColor: '#D9DFE6', overflow: 'hidden' }}>
                        <View style={{ width: '70%', height: '100%', backgroundColor: banho.statusColor, borderRadius: 2 }} />
                      </View>
                    ) : (
                      <View style={[styles.statusBar, { backgroundColor: banho.statusColor }]} />
                    )}
                  </View>
                </TouchableOpacity>
                {expandedBanhoId === banho.id && (
                  <View style={{ alignItems: 'center', marginTop: 12, width: '100%' }}>
                    <Image 
                      source={require('../../../assets/images/BanhoDetalhes.png')} 
                      style={styles.banhoDetalheImage} 
